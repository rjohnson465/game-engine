...
var currentBonusString = ds_map_find_first(skill.bonusesMap);
for (var i = 0; i < ds_map_size(skill.bonusesMap); i++) {
	...
	if skill.object_index == obj_skill_hpmax || skill.object_index == obj_skill_staminamax {
		var bonus =	ds_map_find_value(skill.bonusesMap,currentBonusString)
		var bonusNext = ds_map_find_value(skill.nextLevelBonusesMap,currentBonusString);
		...
		var bonusString = "+ " + string(ds_map_find_value(skill.bonusesMap,currentBonusString)) + currentBonusString;
		draw_text(skillDescriptionCol1X,descriptionHandleY2+((i+1)*25),bonusString);
		var nextLevelBonusString = "+ " + string(ds_map_find_value(skill.nextLevelBonusesMap,currentBonusString)) + currentBonusString;
		draw_text(skillDescriptionCol2X,descriptionHandleY2+((i+1)*25),nextLevelBonusString);
	}
	currentBonusString = ds_map_find_next(skill.bonusesMap,currentBonusString);
}
