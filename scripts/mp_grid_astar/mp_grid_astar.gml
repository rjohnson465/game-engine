...
while !ds_priority_empty(list_open)
{
    ...
    for (i = 0; i < 8; i++)
    {
		...
		// If standing at xx / yy coordinate would collide with a solid, this cell is not an option
		/*var oldX = x; var oldY = y;
		x = (xx*cellwidth)+(.5*cellwidth); y = (yy*cellheight)+.5*cellheight;
		if place_meeting_layer(x, y, obj_solid_parent) {
			//Destroy the succesor.
            ds_list_destroy(succesor);
			x = oldX; y = oldY;
			//Enters next loop.
