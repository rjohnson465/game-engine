...
with owner {
	...
	if	state != CombatantStates.Dodging && 
		(type == CombatantTypes.Player && attackObject.owner.type == CombatantTypes.Enemy
		|| type == CombatantTypes.Ally && attackObject.owner.type == CombatantTypes.Enemy 
		|| type == CombatantTypes.Enemy && attackObject.owner.type == CombatantTypes.Ally
		|| type == CombatantTypes.Enemy && attackObject.owner.type == CombatantTypes.Player) 
		&& attackObject.id != id
	{
		...
		if ds_list_find_index(beenHitWith,attackObject.id) == -1 {
		
			ds_list_add(attackObject.combatantsHit,id);
		
			// limit number of combatants hit per attack based on num targets an attack can hit
			if	attackObject.owner.type == CombatantTypes.Player {
			
				if !attackObject.isSpell && ds_list_size(attackObject.combatantsHit) > attackObject.weapon.numberOfTargets {
					exit;
				} else if attackObject.isSpell && ds_list_size(attackObject.combatantsHit) > attackObject.spell.numberOfTargets {
					exit;
				}
			}
			else if attackObject.owner.type != CombatantTypes.Player 
				&& ds_list_size(attackObject.combatantsHit) > attackObject.attackData.numberOfTargets {
					exit;
				}
		
			ds_list_add(beenHitWith,attackObject.id);
		
			// run to get __x and __y (collision point where attack meet this combatant)
			script_execute(scr_collision_point,id,attackObject.id);
		
			var assailant = attackObject.owner;
			var damage = 0;
			...
			if assailant.type == CombatantTypes.Player {
				...
				if !attackObject.spell {
					itemHitWith = attackObject.weapon;
					attackNumber = attackObject.attackNumber;
					attackNumberInChain = attackObject.attackNumberInChain;
				} else {
					itemHitWith = attackObject.spell;
					spell = attackObject.spell;
					attackNumber = 1;
					attackNumberInChain = 1;
					percentCharged = attackObject.percentCharged;
				}
			...
			// enemies / allies have their attack damages pre-loaded
				...
				attackNumber = attackObject.attackNumber;
				attackNumberInChain = attackObject.attackNumberInChain;
				var isRanged = attackObject.isRanged;
				var attackChain = isRanged ? attackObject.owner.rangedAttacks[attackNumber-1] : attackObject.owner.meleeAttacks[attackNumber-1];
			...
			for (var i = 0; i < size; i++) {
				...
				// blood for physical, elemental particles attackObjectwise
				if	damageBase > 1 {
			...
			if attackObject.isRanged || (attackObject.isSpell && spell.name != "aoe") {
				instance_destroy(attackObject,false);
			}
