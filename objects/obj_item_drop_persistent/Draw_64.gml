...
// loot ui
if isBeingLooted && ds_exists(items,ds_type_list) && ds_list_size(items) != 0 && instance_exists(selectedItem) {
	...
	if selectedItem == noone selectedItem = ds_list_find_value(items,0);
	...
	if gamepad_is_connected(global.player.gamePadIndex) {
		w += drawPrompt("Loot Item",Input.F,promptsX+w,promptsY, c_white, 250);
	} else {
		w += drawPrompt("Loot Item",[Input.LMB,Input.F],promptsX+w,promptsY, c_white, 250);
	}
}